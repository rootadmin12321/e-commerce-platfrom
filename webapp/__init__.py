...
def cart():
    ...
    if signedin is True:
        ...
        # Guest cart
        signedin = False
        session_cart = session.get('cart', {})  # {variant_id: quantity}

        # get a list of all the variant IDs
        ...
        # append quantity to each item
        for i in range(len(variant_details)):
            variant_id = variant_ids[i]
            quantity = session_cart.get(str(variant_id), 1)
            variant_details[i] = variant_details[i] + (quantity,)

        return render_template(
            'cart.html',
            guest_cart=variant_details,
            signedin=False,
...
def checkout():
    ...
    if is_logged:
        ...
        cart = get_cart(userid)   # fetch from DB
        return render_template(
            "checkout.html",
            cart=cart,
            signedin=True,
        ...
        # Guest checkout
        session_cart = session.get('cart', {})  # {variant_id: quantity}
        variant_ids = [int(i) for i in session_cart.keys()]
